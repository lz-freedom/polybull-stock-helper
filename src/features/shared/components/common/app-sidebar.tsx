...
export function AppSidebar({ locale }: AppSidebarProps) {
    ...
    return (
        <>
            <aside
                className={cn(
                    'flex flex-col h-full bg-[#F6F7F9] dark:bg-[#ffffff0a] border-r border-[#E6EAF0] dark:border-[#2A2F36] transition-all duration-300',
            ...
            >
                ...
                <div className="px-2 py-2 space-y-2 border-t border-[#E6EAF0] dark:border-[#2A2F36]">
                    ...
                    {!collapsed && user && (
                        <button
                            type="button"
                            className="relative flex h-12 w-full items-center justify-start gap-3 rounded-[8px] px-4 py-3 text-sm text-[#87664E] transition-all duration-200 ease-linear hover:opacity-90"
                        >
                            <div
                                className="absolute left-0 top-0 z-0 h-full w-full rounded-[8px] bg-cover bg-center"
                                style={{ backgroundImage: `url(${inviteFriendsBg.src})` }}
                            />
                            <UserPlus className="z-10 h-4 w-4" />
                            <span className="z-10 font-medium">{t('shareWithFriends')}</span>
                        </button>
                    )}
                    ...
                    {user ? (
                        <div className={cn('flex items-center gap-3', collapsed ? 'justify-center px-2' : '')}>
                            <div className="h-9 w-9 rounded-full bg-emerald-700 text-white flex items-center justify-center font-semibold text-sm">
                                {userInitial}
                            </div>
                            {!collapsed && (
                            ...
                            {!collapsed && (
                                <Button
                                    ...
                                    variant="outline"
                                    size="sm"
                                    className="rounded-full border-rose-300 text-rose-500 hover:bg-rose-50 px-2 h-6 text-xs"
                                >
                                        ...
                                        <DropdownMenuSubContent>
                                            {themeOptions.map((opt) => (
                                                ...
                                                >
                                                    ...
                                                    <span>{t(opt.labelKey)}</span>
                                                    {theme === opt.value && <ChevronRight className="ml-auto h-4 w-4 opacity-50" />}
                                                </DropdownMenuItem>
                                            ))}
                                        </DropdownMenuSubContent>
                                    </DropdownMenuSub>
                                    <DropdownMenuSub>
                                        ...
                                        <DropdownMenuSubContent>
                                            {localeOptions.map((opt) => (
                                                ...
                                                >
                                                    <span>{opt.label}</span>
                                                    {locale === opt.value && <ChevronRight className="ml-auto h-4 w-4 opacity-50" />}
                                                </DropdownMenuItem>
                                            ))}
                                        </DropdownMenuSubContent>
                                    </DropdownMenuSub>
