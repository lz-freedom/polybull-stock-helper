...
// 提取 mastra 命名空间的数据 parts
function extractMastraDataParts(parts: unknown): DataMessagePart[] {
  if (!Array.isArray(parts)) return [];

  return parts.filter((part): part is DataMessagePart => {
    return (
      part &&
      typeof part === 'object' &&
      'type' in part &&
      ...
      <div className="aui-assistant-message-content wrap-break-word px-2 text-foreground leading-relaxed">
        ...
        <MessagePrimitive.Content>
          {(content) => {
            ...
            const mastraParts = extractMastraDataParts(parts);
            if (mastraParts.length === 0) return null;
            return (
              <div className="mt-3 border-l-2 border-muted pl-3">
                <DataPartList parts={mastraParts} />
              </div>
            );
          }}
        </MessagePrimitive.Content>
